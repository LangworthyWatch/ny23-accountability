...
| `/social-post [entry-or-topic]` | **Caption + card** for a fact-check in the house scorecard format (headline-first, verdict-consistent); runs the ≤2200-char and em-dash checks; builds the card on `social-media/lib/card.py`, then runs a **card audit** (stat-first vs. quote-dense, dead space, color logic) against precedent cards before it's committed or shown | A ready-to-post caption (with char count) + an audited, rendered PNG (Desktop copy) |
